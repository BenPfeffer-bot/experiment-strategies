...
def compute_vwma(series: pd.Series, volume: pd.Series, length: int) -> pd.Series:
    ...
    pv = series * volume
    vwma = (
        pv.rolling(window=length, min_periods=length).sum()
        / volume.rolling(window=length, min_periods=length).sum()
    )
    return vwma


def compute_fibo_bollinger_bands(
    df: pd.DataFrame, length: int = 200, src: str = None, mult: float = 3.0
) -> pd.DataFrame:
    """
...
def plot_fibo_bollinger_bands(
    df: pd.DataFrame, bands: pd.DataFrame, filename: str = "fibo_bollingbands.png"
):
    """
    ...
    plt.plot(
        bands.index, bands["upper_1"], label="Upper 0.236", color="white", linewidth=1
    )
    plt.plot(
        bands.index, bands["upper_2"], label="Upper 0.382", color="white", linewidth=1
    )
    plt.plot(
        bands.index, bands["upper_3"], label="Upper 0.5", color="white", linewidth=1
    )
    plt.plot(
        bands.index, bands["upper_4"], label="Upper 0.618", color="white", linewidth=1
    )
    plt.plot(
        bands.index, bands["upper_5"], label="Upper 0.764", color="white", linewidth=1
    )
    ...
    plt.plot(
        bands.index, bands["lower_1"], label="Lower 0.236", color="white", linewidth=1
    )
    plt.plot(
        bands.index, bands["lower_2"], label="Lower 0.382", color="white", linewidth=1
    )
    plt.plot(
        bands.index, bands["lower_3"], label="Lower 0.5", color="white", linewidth=1
    )
    plt.plot(
        bands.index, bands["lower_4"], label="Lower 0.618", color="white", linewidth=1
    )
    plt.plot(
        bands.index, bands["lower_5"], label="Lower 0.764", color="white", linewidth=1
    )
    plt.plot(
        bands.index, bands["lower_6"], label="Lower 1.0", color="green", linewidth=2
    )
...
if __name__ == "__main__":
    ...
    for ticker in TICKERS:
        ...
        plot_fibo_bollinger_bands(
            df_sample,
            bands,
            filename=f"{FIGURES_DIR}/fibo_bollingbands/{ticker}_fibo_bollingbands.png",
        )
