    ...
    for i in range(len(df)):
        ...
        if is_uptrend and (
            ...
            or (
                ...
                if TREND_MODE == "MACD - Signal"
                else df["macd"].iloc[i - 1] <= 0
            )
        ):
        ...
        if trigger_series.iloc[i]:
            ...
            if is_uptrend:
                forecasts_upper, forecasts_mid, forecasts_lower = memory_up.forecast(
                    uptrend_init_price,
                    FCAST,
                    UP_PERCENTILE,
                    MID_PERCENTILE,
                ...
                forecasts_upper, forecasts_mid, forecasts_lower = memory_down.forecast(
                    downtrend_init_price,
                    FCAST,
                    UP_PERCENTILE,
                    MID_PERCENTILE,
                    DN_PERCENTILE,
                ...
                forecasts.append(
                    {
                        "trigger_index": i,
                        "trend": trend_str,
                        "init_price": uptrend_init_price
                        if is_uptrend
                        else downtrend_init_price,
...
if __name__ == "__main__":
    ...
    for ticker in TICKERS:
        ...
        for event in forecast_events:
            print(
                f"Trigger at bar index {event['trigger_index']}, trend: {event['trend']}, "
                f"Reference Price: {event['init_price']:.2f}"
            )

        # Plot the results
        plot_and_save_forecasting(
            df_sample,
            forecast_events,
            filename=f"{FIGURES_DIR}/macd_ml/{ticker}_macd_ml_forecast.png",
        )
