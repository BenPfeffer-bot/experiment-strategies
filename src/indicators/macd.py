...
def compute_macd(df: pd.DataFrame, 
    ...
    Parameters:
      ...
      fast_length : int
         The period for the fast EMA (default 12).
      slow_length : int
         The period for the slow EMA (default 26).
      signal_length : int
         The period for the signal SMA (default 9).

    Returns:
      A DataFrame with columns: "macd", "signal", and "hist", indexed like df.
    """
    if 'close' not in df.columns:
        raise ValueError("DataFrame must contain 'close' column.")
    
    # Calculate the fast and slow exponential moving averages using pandas ewm
    fast_ema = df['close'].ewm(span=fast_length, adjust=False).mean()
    slow_ema = df['close'].ewm(span=slow_length, adjust=False).mean()
    
    macd = fast_ema - slow_ema
    # Signal line as a simple moving average of MACD
...
def plot_macd(df: pd.DataFrame,
    ...
    if macd_color_change:
        seg_colors = ["lime" if m >= s else "red" for m, s in zip(macd_series, signal_series)]
    else:
    ...
    if show_dots and show_macd:
        ...
        cross_indices = np.where(crosses)[0] + 1  # +1 gives the index of the crossing bar
        ...
        for idx in cross_indices:
            ...
            if macd_color_change:
                dot_color.append("lime" if macd_series[idx] >= signal_series[idx] else "red")
            else:
                dot_color.append("red")
        ax.scatter(x[cross_indices], signal_series[cross_indices], color=dot_color, s=100, zorder=5, label="Cross Dot")
    ...
    if hasattr(df.index, 'strftime'):
        ax.set_xticklabels(df.index.strftime("%Y-%m-%d")[::max(1, len(x)//10)], rotation=45, ha='right')
    else:
...
if __name__ == "__main__":
    ...
    for ticker in TICKERS:
        ...
        # Compute MACD components
        macd_data = compute_macd(df_sample, fast_length=12, slow_length=26, signal_length=9)
        
        if not os.path.exists(f"{FIGURES_DIR}/macd"):
            os.makedirs(f"{FIGURES_DIR}/macd")
        
        # Plot the indicator:
        plot_macd(df_sample, macd_data,
                  show_macd=True,
                  show_dots=True,
                  show_hist=True,
