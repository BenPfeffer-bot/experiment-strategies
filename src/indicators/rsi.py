...
class RSIMultiLength:
    def __init__(self, max_length=20, min_length=10, overbought=80, oversold=20):
        """
        Parameters:
        • max_length: int
    ...
    def compute(self, src: pd.Series):
        ...
        for t in range(n_bars):
            ...
            if prev_buy_rsi is None:
                ...
                sell_rsi = prev_sell_rsi + (oversell / self.N) * (
                    avg_rsi - prev_sell_rsi
                )
        ...
        avg_rsi_series = pd.Series(
            avg_rsi_list, index=src.index, name="Average Multi-Length RSI"
        )
        buy_rsi_series = pd.Series(buy_rsi_list, index=src.index, name="Upper Channel")
        sell_rsi_series = pd.Series(
            sell_rsi_list, index=src.index, name="Lower Channel"
        )
        overbought_pct_series = pd.Series(
            overbought_pct_list, index=src.index, name="Overbought %"
        )
        oversold_pct_series = pd.Series(
            oversold_pct_list, index=src.index, name="Oversold %"
        )
    ...
    def plot_and_save(
        self, src: pd.Series, computed: dict, filename="rsi_multilength.png"
    ):
        """
...
if __name__ == "__main__":
    ...
    for ticker in TICKERS:
        ...
        rsi_indicator.plot_and_save(
            prices,
            computed_values,
            filename=f"{FIGURES_DIR}/rsi/{ticker}_rsi_multilength.png",
        )

        # Print some sample output for the last 5 data points
        print(
            "Date       | Average RSI | Upper Channel | Lower Channel | Overbought % | Oversold %"
        )
