...
class DataIntegrator:
    ...
    def clean_market_data(
        self, missing_method: str = "ffill", z_threshold: float = 3.0
    ) -> pd.DataFrame:
        """
    ...
    def combine_data(self) -> pd.DataFrame:
        ...
        fundamental_static = (
            fundamental_df.iloc[0].to_dict() if not fundamental_df.empty else {}
        )
...
if __name__ == "__main__":
    ...
    for ticker in TICKERS:
        market_df = pd.read_csv(f"{RAW_DATA_DIR}/{ticker}_data.csv")
        fundamental_data = pd.read_json(
            f"{FUNDAMENTAL_DATA_DIR}/{ticker}_fundamentals.json"
        )
        ...
        combined_data.to_csv(
            f"{integrator.output_dir}/{ticker}_combined_data.csv", index=True
        )
